...
const RecommendedAlbums = () => {
  ...
  const fetchRecommendedAlbums = async () => {
    ...
    try {
      ...
      if (data.recommendations?.albums) {
  ...
  const fetchAIRecommendations = async () => {
    ...
    if (aiRateLimitInfo && !aiRateLimitInfo.canMakeRequest) {
      const timeRemaining =
        aiRateLimitInfo.timeUntilReset || countdown || 60;
      showToast.warn(
        `AI rate limit reached! Please wait ${timeRemaining} seconds before trying again.`
      );
      return;
    }
    ...
    try {
      ...
      if (data.recommendations?.albums) {
        ...
        const newAlbums = data.recommendations.albums.filter(
      ...
      if (isAxiosError<ApiErrorResponse>(err) && err.response?.status === 429) {
        ...
        setError(
          `AI requests exhausted. Try again in ${timeRemaining} seconds.`
        );
      } else {
  ...
  const renderAlbumCard = (album: AlbumCard) => (
    <div
      key={album.id}
      className="carousel-item w-[280px] sm:w-[320px] lg:w-[330px] flex-shrink-0"
    >
  ...
  if (!albums.length && !loading && !aiLoading) {
    return (
        ...
        <div className="flex flex-col items-center justify-center py-12">
          ...
          {aiLoading && (
            <p className="text-gray-400 text-sm text-center">
              🤖 AI is discovering personalized albums...
              <br />
              <span className="text-purple-400 text-xs">
                Using advanced taste analysis
              </span>
            </p>
          )}
          {loading && isUsingFallback && (
            <p className="text-gray-400 text-sm text-center">
              AI system is processing your taste profile...
              <br />
              <span className="text-yellow-400 text-xs">
                This may take a moment longer
              </span>
            </p>
          )}
        </div>
