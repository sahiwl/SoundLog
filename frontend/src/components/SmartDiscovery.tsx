...
const SmartDiscovery = () => {
  ...
  const [aiRateLimitInfo, setAiRateLimitInfo] =
    useState<AIRateLimitInfo | null>(null);
  const [countdown, setCountdown] = useState(0);

  useEffect(() => {
    if (
      aiRateLimitInfo &&
      !aiRateLimitInfo.canMakeRequest &&
      aiRateLimitInfo.timeUntilReset > 0
    ) {
  ...
  const fetchMoodRecommendations = async (
    ...
    try {
      ...
      if (isAxiosError<ApiErrorResponse>(err) && err.response?.status === 429) {
        const rateLimitInfo = err.response.data?.aiRateLimitInfo;
        if (rateLimitInfo) {
          setAiRateLimitInfo(rateLimitInfo);
          setError(
            `AI requests exhausted. Try again in ${rateLimitInfo.timeUntilReset || 60} seconds.`
          );
        } else {
  ...
  const handleMoodSelect = (mood: Mood) => {
    if (aiRateLimitInfo && !aiRateLimitInfo.canMakeRequest && countdown > 0) {
      setError(
        `Please wait ${countdown} seconds before requesting more AI recommendations.`
      );
      return;
    }
  ...
  const renderAlbumCard = (album: AlbumCard) => (
    <div
      key={album.id}
      className="carousel-item w-[280px] sm:w-[320px] lg:w-[330px] shrink-0"
    >
  ...
  return (
    <section className="mb-12">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mb-6">How are you feeling today?</p>
          ...
          countdown > 0 && (
            <div className="mb-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3">
              ...
              <p className="text-yellow-200 text-xs mt-1">
                You&apos;ve reached the AI request limit. Please wait before
          ...
          {(Object.keys(MOOD_ICONS) as Mood[]).map((mood) => {
            ...
            return (
              <button
                ...
                disabled={Boolean(isRateLimited && !isSelected)}
                className={`
        ...
        <div className="text-center py-12 bg-gray-800/30 rounded-lg border border-gray-700">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-gray-400 text-sm">
              Select how you&apos;re feeling above to get music recommendations
              powered by AI, tailored to your current vibe.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 text-center">
          ...
          {error.includes("exhausted") || error.includes("wait") ? (
            <p className="text-red-200 text-xs mt-2">
              Rate limit info: {aiRateLimitInfo?.remainingRequests || 0}{" "}
              requests remaining
            </p>
          ) : (
            <button
              onClick={() =>
                selectedMood && fetchMoodRecommendations(selectedMood)
              }
        ...
        <div>
          <div className="mb-4">
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
              ...
              {aiRateLimitInfo && (
                <p className="text-blue-200 text-xs mt-1">
                  AI requests remaining: {aiRateLimitInfo.remainingRequests}/
                  {aiRateLimitInfo.maxRequests}
                </p>
              )}
            </div>
          </div>

          {recommendations.recommendations?.albums?.length ? (
            <HorizontalCarousel>
          ...
          <div className="text-center mt-4">
            <button
              onClick={() => {
                if (
                  ...
                  setError(
                    `Please wait ${countdown} seconds before requesting more recommendations.`
                  );
                  return;
                }
                if (selectedMood) fetchMoodRecommendations(selectedMood);
              }}
              disabled={
                loading ||
                Boolean(
            ...
            >
              Get More {selectedMood?.charAt(0).toUpperCase()}
              {selectedMood?.slice(1)} Vibes
            </button>
          </div>
        </div>
      )}
