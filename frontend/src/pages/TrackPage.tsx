...
import type { TrackPageResponse, CachedTrack } from "../types/api";

const TrackPage = () => {
  const { trackId } = useParams();
  const [trackData, setTrackData] = useState<CachedTrack | null>(null);
  ...
  const fetchTrackDetails = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const response = await axiosInstance.get<TrackPageResponse>(
  ...
  if (!trackData || !trackId) return <p>No track data found.</p>;
            ...
            <div className="lg:col-span-2">
              <div className="mb-6">
                ...
                <p className="text-gray-400 mt-2">
                  ...
                  <a
                    href={`/album/${trackData.album?.spotifyId}`}
                    ...
                    <div className="flex items-end mb-2">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-sm text-gray-400">
                          Track {trackData.track_number} on album
                        </p>
                      </div>
                    </div>
                  ...
                  {trackData.artists?.map((artist, index) => (
                    <p key={artist.spotifyId}>
                      ...
                      <span className="text-gray-400">
                        {" "}
                        / {index === 0 ? "primary artist" : "feature"}
                      </span>
                    </p>
                  ))}
                  <p>
                    <span className="text-white">
                      {trackData.explicit ? "Yes" : "No"}
                    </span>
