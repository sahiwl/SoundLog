...
try {
    ...
    return res.status(500).json({message: "Server error in followUser controller"})
}
...
const unfollowUser = async (req,res) => {
    ...
    if(!tarUser){
        ...
        tarUser.following == tarUser.following.filter(id=> id.toString()!== targetUserId )
        ...
        return res.status(500).json({message: "server error in unfollowUser controller"})
    }
}
...
const getUserProfile = async (req,res)=>{
    ...
    console.log(`profile fetched for user: ${id}`);
        ...
        return res.status(500).json({message: "Server Error in getUserProfile controller"});
    }
}
